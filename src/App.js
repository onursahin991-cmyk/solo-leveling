...
export default function App() {
  ...
  // Check for in-memory save on mount
  useEffect(() => {
...
Kategoriler: güç(fiziksel güç), dayanıklılık(kardiyo), çeviklik(hız/koordinasyon), irade(zihinsel dayanıklılık), meditasyon(nefes/odak), denge(denge/propriosepsiyon), duyular(algı keskinleştirme).
        ...
        {screen === "assessment" && (
          <div style={{ paddingTop: 40 }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 16 }}>
              {QUESTIONS.map((_, i) => (
                <div key={i} style={{ width: i === currentQ ? 20 : 8, height: 8, borderRadius: 4, background: i < currentQ ? "#4f46e5" : i === currentQ ? "#818cf8" : "#1e293b", transition: "all .3s" }} />
              ))}
            </div>
          </div>
        )}
        ...
        {screen === "profile" && profile && (
          <div style={{ paddingTop: 14, animation: "slideUp .5s ease" }}>
            ...
            <div style={{ background: "#0a0f1a", border: "1px solid #1e293b", padding: "11px 14px", marginBottom: 10 }}>
              ...
              <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, color: "#334155", marginTop: 4, textAlign: "right" }}>{xp % xpThreshold} / {xpThreshold} XP</div>
            </div>
                ...
                return (
                  <div key={k} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, letterSpacing: 2, color: "#475569" }}>
                        {lb[k]}
                        {k === profile.weakest && <span style={{ color: "#ef4444" }}> ↓</span>}
                        {k === profile.strongest && <span style={{ color: "#10b981" }}> ↑</span>}
                      </span>
                      <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 11, fontWeight: 700, color: cl[k] || "#64748b" }}>{v}</span>
                    </div>
                    <div style={{ height: 5, background: "#1e293b", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${Math.min(v, 100)}%`, borderRadius: 3, background: `linear-gradient(to right,${cl[k] || "#64748b"}88,${cl[k] || "#64748b"})`, transition: "width 1.5s cubic-bezier(.4,0,.2,1)" }} />
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{ paddingTop: 14 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <div style={{ padding: "3px 9px", border: `1px solid ${rc}`, color: rc, fontFamily: "'Rajdhani',sans-serif", fontSize: 12, fontWeight: 700 }}>{profile.rank}</div>
            ...
            <div style={{ height: 3, background: "#1e293b", borderRadius: 2, marginBottom: 16, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${xpProgress}%`, background: "linear-gradient(to right,#a16207,#fbbf24)", transition: "width .8s ease" }} />
            </div>
            ...
            {activeTab === "quests" && (
              <div>
                <div style={{ fontSize: 10, letterSpacing: 4, color: "#4f46e5", fontFamily: "'Rajdhani',sans-serif", marginBottom: 12 }}>◆ AKTİF GÖREVLER</div>
                {loading ? (
                  <div style={{ textAlign: "center", padding: "32px", color: "#475569", fontFamily: "'Rajdhani',sans-serif" }}>
                    <div style={{ fontSize: 26, marginBottom: 8 }}>⚔️</div>Yeni görevler hazırlanıyor...
                  </div>
                ) : quests.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "32px", color: "#334155", fontFamily: "'Rajdhani',sans-serif" }}>
                    <div style={{ fontSize: 26, marginBottom: 8 }}>⚔️</div>Tüm görevler tamamlandı!
                  </div>
                ) : quests.map((q, i) => {
                  const cat = CATEGORY_INFO[q.category] || { icon: "◆", color: "#64748b" };
                  return (
                    <div key={q.id || i} className="sl-qcard" style={{ animationDelay: `${i * 0.08}s` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, alignItems: "center" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                          <span style={{ fontSize: 15 }}>{cat.icon}</span>
                          <span style={{ fontSize: 9, letterSpacing: 3, color: DIFF_COLORS[q.difficulty] || "#64748b", fontFamily: "'Rajdhani',sans-serif" }}>[{q.difficulty}]</span>
                          <span style={{ fontSize: 9, letterSpacing: 2, color: cat.color, fontFamily: "'Rajdhani',sans-serif" }}>{(q.category || "").toUpperCase()}</span>
                        </div>
                        <span style={{ color: "#fbbf24", fontFamily: "'Rajdhani',sans-serif", fontSize: 11, fontWeight: 700 }}>+{q.xp} XP</span>
                      </div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0", marginBottom: 4 }}>{q.title}</div>
                      <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 12, color: "#64748b", marginBottom: 9 }}>{q.description}</div>
                      <div style={{ background: "#0d1117", border: "1px solid #1e293b", padding: "8px 12px", marginBottom: 11, fontFamily: "'Rajdhani',sans-serif", fontSize: 12, color: "#94a3b8" }}>📋 {q.task}</div>
                      <button className="sl-btn" style={{ fontSize: 10, padding: "9px" }} onClick={() => completeQuest(q)}>◆ TAMAMLANDI ◆</button>
                    </div>
                  );
                })}
              </div>
            ...
            {activeTab === "mind" && (
              <div>
                {/* Meditasyon */}
                <div style={{ background: "linear-gradient(135deg,#060d10,#040c10)", border: "1px solid #0e3a47", padding: "18px", marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                    <span style={{ fontSize: 20 }}>🧘</span>
                    <div>
                      <div style={{ fontSize: 9, letterSpacing: 4, color: "#06b6d4", fontFamily: "'Rajdhani',sans-serif" }}>MEDİTASYON</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0" }}>Zihin Sakinleştirme</div>
                    </div>
                  </div>
                  {MEDITATION_EXERCISES.map((ex, i) => (
                    <div key={i} className="sl-excard" style={{ background: "#030a0d", border: "1px solid #0e3a47" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 13, fontWeight: 700, color: "#e2e8f0" }}>{ex.name}</span>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, color: "#fbbf24" }}>+{ex.xp} XP</span>
                      </div>
                      <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 12, color: "#64748b", marginBottom: 8 }}>{ex.desc}</div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, color: "#06b6d4" }}>⏱ {ex.time}</span>
                        <button className="sl-btn-sm" style={{ borderColor: "#06b6d4", color: "#67e8f9" }} onClick={() => completeStaticQuest(ex.xp)}>TAMAMLA</button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {/* Denge */}
                <div style={{ background: "linear-gradient(135deg,#0a0900,#080700)", border: "1px solid #3d2e00", padding: "18px", marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                    <span style={{ fontSize: 20 }}>⚖️</span>
                    <div>
                      <div style={{ fontSize: 9, letterSpacing: 4, color: "#f59e0b", fontFamily: "'Rajdhani',sans-serif" }}>DENGE</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0" }}>Propriosepsiyon</div>
                    </div>
                  </div>
                  {BALANCE_EXERCISES.map((ex, i) => (
                    <div key={i} className="sl-excard" style={{ background: "#060500", border: "1px solid #3d2e00" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 13, fontWeight: 700, color: "#e2e8f0" }}>{ex.name}</span>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, color: "#fbbf24" }}>+{ex.xp} XP</span>
                      </div>
                      <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 12, color: "#64748b", marginBottom: 8 }}>{ex.desc}</div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, color: "#f59e0b" }}>⏱ {ex.time}</span>
                        <button className="sl-btn-sm" style={{ borderColor: "#f59e0b", color: "#fbbf24" }} onClick={() => completeStaticQuest(ex.xp)}>TAMAMLA</button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {/* Duyular */}
                <div style={{ background: "linear-gradient(135deg,#0a0007,#080006)", border: "1px solid #4a1030", padding: "18px", marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                    <span style={{ fontSize: 20 }}>👁️</span>
                    <div>
                      <div style={{ fontSize: 9, letterSpacing: 4, color: "#ec4899", fontFamily: "'Rajdhani',sans-serif" }}>DUYULAR</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0" }}>Algı Keskinleştirme</div>
                    </div>
                  </div>
                  {SENSE_EXERCISES.map((ex, i) => (
                    <div key={i} className="sl-excard" style={{ background: "#060004", border: "1px solid #4a1030" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 13, fontWeight: 700, color: "#e2e8f0" }}>{ex.name}</span>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, color: "#fbbf24" }}>+{ex.xp} XP</span>
                      </div>
                      <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 12, color: "#64748b", marginBottom: 8 }}>{ex.desc}</div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, color: "#ec4899" }}>⏱ {ex.time}</span>
                        <button className="sl-btn-sm" style={{ borderColor: "#ec4899", color: "#f9a8d4" }} onClick={() => completeStaticQuest(ex.xp)}>TAMAMLA</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {activeTab === "chat" && (
              <div>
                <div style={{ fontSize: 10, letterSpacing: 4, color: "#4f46e5", fontFamily: "'Rajdhani',sans-serif", marginBottom: 12 }}>◆ SİSTEM REHBERİ</div>
                <div style={{ background: "linear-gradient(135deg,#0d1117,#0a0f1e)", border: "1px solid #1e293b", padding: 16 }}>
                  <div style={{ maxHeight: 340, overflowY: "auto", marginBottom: 12 }}>
                    {chatHistory.length === 0
                      ? <div style={{ color: "#334155", fontFamily: "'Rajdhani',sans-serif", fontSize: 13, fontStyle: "italic" }}>"Antrenman tavsiyesi, meditasyon rehberi, denge ve duyular hakkında sor..."</div>
                      : chatHistory.map((m, i) => (
                        <div key={i} style={{ marginBottom: 9, padding: "9px 12px", background: m.role === "user" ? "#0f172a" : "#1e1b4b22", border: `1px solid ${m.role === "user" ? "#1e293b" : "#312e8133"}`, fontFamily: "'Rajdhani',sans-serif", fontSize: 13, color: m.role === "user" ? "#94a3b8" : "#a5b4fc", lineHeight: 1.6 }}>
                          <div style={{ fontSize: 9, letterSpacing: 2, color: m.role === "user" ? "#475569" : "#4f46e5", marginBottom: 3 }}>{m.role === "user" ? "SEN" : "SİSTEM"}</div>
                          {m.content}
                        </div>
                      ))}
                    <div ref={chatEndRef} />
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    />
                    <button className="sl-btn" onClick={askAI} disabled={loading} style={{ width: "auto", fontSize: 10, padding: "0 14px", whiteSpace: "nowrap", opacity: loading ? 0.5 : 1 }}>
                      {loading ? "..." : "GÖNDER"}
                    </button>
                  </div>
                </div>
            ...
            {activeTab === "history" && (
              <div>
                <div style={{ fontSize: 10, letterSpacing: 4, color: "#4f46e5", fontFamily: "'Rajdhani',sans-serif", marginBottom: 12 }}>◆ TAMAMLANAN GÖREVLER ({completedQuests.length})</div>
                {completedQuests.length === 0
                  ? <div style={{ color: "#334155", fontFamily: "'Rajdhani',sans-serif", fontSize: 13, textAlign: "center", padding: "32px" }}>Henüz tamamlanan görev yok.</div>
                  : [...completedQuests].reverse().map((q, i) => (
                    <div key={i} style={{ background: "#0a0f1a", border: "1px solid #0f172a", padding: "9px 13px", marginBottom: 5, fontFamily: "'Rajdhani',sans-serif", fontSize: 12, display: "flex", gap: 9, alignItems: "center" }}>
                      <span style={{ color: "#10b981", fontSize: 13 }}>✓</span>
                      <span style={{ color: "#334155" }}>{q}</span>
                    </div>
                  ))}
                {completedQuests.length > 0 && (
                  <div style={{ marginTop: 14, background: "#0d1117", border: "1px solid #1e293b", padding: "13px 15px" }}>
                    <div style={{ fontSize: 9, letterSpacing: 3, color: "#334155", fontFamily: "'Rajdhani',sans-serif", marginBottom: 10 }}>TOPLAM İSTATİSTİK</div>
                    <div style={{ display: "flex", gap: 24 }}>
                      {[{ val: completedQuests.length, label: "GÖREV", color: "#fbbf24" }, { val: xp, label: "TOPLAM XP", color: "#818cf8" }, { val: currentLevel, label: "SEVİYE", color: rc }].map((s, i) => (
                        <div key={i}>
                          <div style={{ fontSize: 22, fontWeight: 700, color: s.color, fontFamily: "'Cinzel',serif" }}>{s.val}</div>
                          <div style={{ fontSize: 9, color: "#334155", letterSpacing: 2, fontFamily: "'Rajdhani',sans-serif" }}>{s.label}</div>
                        </div>
                      ))}
                    </div>
                  </div>
