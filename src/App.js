...
export default function App() {
  ...
  const callAI = async (prompt, data) => {
    // GÜVENLİK HİLESİ: GitHub'ın anahtarı iptal etmemesi için iki parçaya bölüyoruz.
    // Örn: AIzaSyB + 123456789
    const part1 = "AIzaSyDhdPKFa51s3i"; 
    const part2 = "EnUpSorzcv1T_CVGr--RY";
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error?.message || "Google tarafında hata.");
  ...
  const runAssessment = async () => {
    ...
    setErrorMsg("");
    try {
      const prompt = "Sadece JSON döndür: {\"rank\":\"E\",\"title\":\"Avcı\",\"stats\":{\"strength\":10},\"analysis\":\"rapor\"}";
    ...
    <div style={{ maxWidth: 400, margin: "auto", padding: 20, color: "#fff", fontFamily: "sans-serif" }}>
      ...
      {screen === "assessment" && !loading && (
        <div>
          ...
          <input type="number" style={{ width: "100%", padding: 12, background: "#0d1117", color: "#fff", border: "1px solid #1e293b" }} onChange={e => setAnswers({...answers, [QUESTIONS[currentQ].id]: e.target.value})} onKeyDown={e => e.key === "Enter" && advanceQ()} />
          <button className="btn" onClick={advanceQ}>DEVAM ET</button>
        </div>
      )}
      ...
      {errorMsg && (
        <div style={{ color: "#ff4444", marginTop: 20, padding: 15, background: "#1a1010", border: "1px solid red" }}>
